...
/**
 * Two types used to facilitate receiving events from an event bus.
 */
// export type EventBusSubscriberState<Bus, Context extends FSM<never>> = State &
//     (Bus extends EventBus<infer E>
//         ? keyof E extends string
//             ? { [T in keyof E as `handle${Capitalize<T>}Event`]: (context: Context, event: E[T]) => void }
//             : never
//         : never);
// export type EventBusSubscriberFSM<S extends State, Bus> = FSM<S> &
//     (Bus extends EventBus<infer E>
//         ? keyof E extends string
//             ? { [T in keyof E as `handle${Capitalize<T>}Event`]: (event: E[T]) => void }
//             : never
//         : never);
