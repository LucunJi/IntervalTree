...
$(() => {
    ...
    populateForm.on('submit', (event) => {
        plotboard.populateSegments(
            parseInt($('#populate-count').val() as string)
        );
    ...
    algoBtn.start.on('click', () => {
        if (algoState.state === 'add' && plotboard.getSegments().length === 0)
            return;
    ...
    plotboard.setSortingEnd(
        $('[name=sort-endpoint]:checked').val() as 'left' | 'right'
    );
    $('[name=sort-endpoint]')
        .each((idx, ele) => {
            $(ele).on('change', (event) => {
                plotboard.setSortingEnd(
                    $(event.target).val() as 'left' | 'right'
                );
            });
        })
        .prop('disabled', true);
    ...
    function updateRecursionStatus(board: TreeBoard, currNode: TreeNode) {
        ...
        if (board.getMode() === 'build') {
            for (const node of board.tree.nodes) {
                if (node === undefined) continue;
                plotboard.setMedianVisible(
                    node.median,
                    board.graphNode(node)!.getAttribute('visible')
                );
            }
        } else {
            // show the median for the current node and its ancestors
            for (
                let node: TreeNode | undefined = currNode;
                node !== undefined;
                node = node.parent
            ) {
                ...
                if (x < currNode.median) {
                    intersections = node.segmentsLeftSorted.filter(
                        (s) => hrange(s).left <= x
                    );
            ...
            for (
                let i = (1 << currNode.depth) - 1 + currNode.peerIdx + 1;
                i < board.tree.nodes.length;
                i++
            ) {
    ...
    algoState.onChange((newState) => {
        if (newState === 'build' && treeboard === undefined) {
            ...
            treeboard.onHoverNode((event) => {
                if (event.prevNode !== undefined)
                    plotboard.removeMedian('hover');
                if (event.node !== undefined) {
                    plotboard.addMedian(
                        ...
                        treeboard!
                            ...
                            .getAttribute('highlightFillColor'),
                        true,
                        'hover'
                    );
                }
