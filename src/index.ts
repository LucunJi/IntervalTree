...
import { NewSegmentEvent, PlotBoard, QueryLineChangeEvent } from './components/plotboard';
...
import { EventBus, EventBusImpl } from './utils/patterns';
import { DrawState, IntervalTreeFSM, StateChangeEvent } from './states';

export type PopulateSegmentEvent = { count: number };
export type AlgoStartButtonPressedEvent = Record<string, never>;
export const BUS: EventBus<{
    stateChange: StateChangeEvent;
    algoStartButtonPressed: AlgoStartButtonPressedEvent;

    populateSegment: PopulateSegmentEvent;
...
$(() => {
    ...
    BUS.on('populateSegment', FSM.getGenericEventHandler('populateSegment'));
    BUS.on('algoStartButtonPressed', FSM.getGenericEventHandler('algoStartButtonPressed'))

    // TODO: combine populate button into plotboard
    $('#populate').on('submit', (e) => {
        BUS.emit('populateSegment', { count: parseInt($('#populate-count').val() as string) });
        e.preventDefault();
    });
    BUS.on('stateChange', (e) => {
        $('#populate :input').prop('disabled', e.state === 'draw');
    });

    $('#algo-buttons button').prop('disabled', true);
    BUS.on('newSegment', () => {
        $('#algo-start').prop('disabled', false);
    });
    $('#algo-start').on('click', () => BUS.emit('algoStartButtonPressed', {}));

    // TODO: combine algorithm controlling buttons with simulator (ControlledSimulator)
});
